...
public partial class DisableArcaneSpellFailureFeature : FeatureWithPatch {
    ...
    [LocalizedString("ToyBox_Features_BagOfTricks_Cheats_DisableArcaneSpellFailureFeature_Description", "Sets the spell failure chance on spells to 0")]
    public override partial string Description { get; }
    [HarmonyPatch(typeof(RuleCastSpell), nameof(RuleCastSpell.ArcaneSpellFailureChance), MethodType.Getter), HarmonyFinalizer]
    private static void RuleCastSpell_ArcaneSpellFailureChance_Patch(ref int __result) {
        __result = 0;
    }
    [HarmonyPatch(typeof(RuleCastSpell), nameof(RuleCastSpell.SpellFailureChance), MethodType.Getter), HarmonyFinalizer]
    private static void RuleCastSpell_SpellFailureChance_Patch(ref int __result) {
        __result = 0;
    }
    [HarmonyPatch(typeof(BlueprintItemArmor), nameof(BlueprintItemArmor.ArcaneSpellFailureChance), MethodType.Getter), HarmonyFinalizer]
    public static void BlueprintItemArmor_ArcaneSpellFailureChance_Patch(ref int __result) {
        __result = 0;
    }
}
