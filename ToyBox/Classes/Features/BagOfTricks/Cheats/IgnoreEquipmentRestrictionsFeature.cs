...
public partial class IgnoreEquipmentRestrictionsFeature : FeatureWithPatch {
    ...
    private static void BlueprintItemEquipment_CanBeEquippedBy_Patch(ref bool __result) {
        __result = true;
    }
    [HarmonyPatch(typeof(ItemEntityArmor), nameof(ItemEntityArmor.CanBeEquippedInternal)), HarmonyPostfix]
    private static void ItemEntityArmor_CanBeEquippedInternal_Patch(ItemEntityArmor __instance, UnitDescriptor owner, ref bool __result) {
    ...
    private static void ItemEntityShield_CanBeEquippedInternal_Patch(ItemEntityShield __instance, UnitDescriptor owner, ref bool __result) {
        if (__instance.Blueprint is BlueprintItemEquipment equip) {
            __result = equip.CanBeEquippedBy(owner);
        }
    }
    [HarmonyPatch(typeof(ItemEntityWeapon), nameof(ItemEntityWeapon.CanBeEquippedInternal)), HarmonyPostfix]
    private static void ItemEntityWeapon_CanBeEquippedInternal_Patch(ItemEntityWeapon __instance, UnitDescriptor owner, ref bool __result) {
        if (__instance.Blueprint is BlueprintItemEquipment equip) {
            __result = equip.CanBeEquippedBy(owner);
        }
    }
