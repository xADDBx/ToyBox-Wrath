...
public partial class LanguagePickerFeature : Feature {
    ...
    private static Browser<CultureInfo> m_Browser = new((CultureInfo ci) => $"{ci.Name} {ci.NativeName} {ci.DisplayName} {ci.EnglishName}", (CultureInfo ci) => $"{ci.Name} {ci.NativeName} {ci.DisplayName} {ci.EnglishName}", [..LocalizationManager.DiscoverLocalizations().Select(name => CultureInfo.GetCultureInfo(name))], null, true, 400);
