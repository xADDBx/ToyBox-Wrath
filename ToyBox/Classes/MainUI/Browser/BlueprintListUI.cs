                    ...
                    using (HorizontalScope()) {
                        ...
                        using (VerticalScope()) {
                            using (HorizontalScope(GUI.skin.button)) {
                                var content = new GUIContent($"{RichText.Yellow(paramBP.Name)}");
                                ...
                                foreach (var entry in selectionData.SelectionsByLevel) {
                                    foreach (var selection in entry.Value) {
                                        ...
                                        using (HorizontalScope()) {
                                            ...
                                            Space(25);
                                            Label(RichText.Yellow($"{entry.Key} ") + RichText.Orange(selection.Name), Width(250));
                                            Space(25);
                                            Label(RichText.Green(selection.Description.StripHTML()));
                                        }
                                    }
                                }
                            }
