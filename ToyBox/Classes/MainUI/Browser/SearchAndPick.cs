    ...
    public static class SearchAndPick {
        ...
        private static readonly NamedTypeFilter[] blueprintTypeFilters = new NamedTypeFilter[] {
            ...
            new NamedTypeFilter<BlueprintItemWeapon>("Weapons", null, (bp) => {
                ...
                if (category != null) return bp.CollationNames(category.ToString(), $"{bp.GetCost().ToBinString(RichText.Yellow("⊙"))}");
                if (type != null) return bp.CollationNames(type.NameSafe(), $"{bp.GetCost().ToBinString(RichText.Yellow("⊙"))}");
